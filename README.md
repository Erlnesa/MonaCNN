...
其中，单张图片预测分类消耗所的时间为：0.06~0.27秒，平均值为0.075秒，众数为0.06秒。运行结果如下图所示。
...
从训练损失曲线(图片见下文)来看，在训练过程中验证集曲线震荡明显，很有可能是过小的batch size造成了每次训练确定的梯度下降方向不准确。因此，如果能增加batch size的值可能会为模型带来更高的分类准确度。同时因为一次读入的样本增多了，所以在一定程度上也可能会加快模型的训练速度。
...
从训练损失曲线来看，在完成20次迭代训练时，模型的训练损失处于明显的下降趋势中。因此，此时的模型处于欠拟合状态，需要进一步增加迭代次数来增加模型的分类准确度。
...
预训练完毕后再由正常的样本集（画面里含有“莫娜”的图片；画面里没有“莫娜”的图片）训练得到最终不会过于关注角色服装和头发颜色的模型。训练损失曲线如下图所示。
